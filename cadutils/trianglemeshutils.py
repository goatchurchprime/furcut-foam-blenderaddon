...
#print("reloading utils.trianglemeshutils")
#import importlib
#import barmesh.tribarmes
#print("should reload barmesh.tribarmes")
#importlib.reload(barmesh.tribarmes)
#TriangleBarMesh = barmesh.tribarmes.TriangleBarMesh
...
class UsefulBoxedTriangleMesh:
    def __init__(self, meshobject, btriangleboxing=True, boriginalindexes=False, transform=None):
        global tbarmesh, tboxing, hitreg, nhitreg
        if boriginalindexes:
            trpts = [ sum(((a+(b,)) for a, b in zip(f.Points, f.PointIndices)), ())  for f in meshobject.Facets ]
        else:
            trpts = [ sum(f.Points, ())  for f in meshobject.Facets ]
        if transform:
            trpts = [ tuple(transform*Vector(p[0],p[1],p[2]))+tuple(transform*Vector(p[3],p[4],p[5]))+tuple(transform*Vector(p[6],p[7],p[8]))  for p in trpts ]
            
        print("building tbarmesh ", len(trpts))
        self.tbarmesh = TriangleBarMesh()
        self.tbarmesh.BuildTriangleBarmesh(trpts)
        assert len(self.tbarmesh.nodes) == meshobject.CountPoints, (len(self.tbarmesh.nodes), meshobject.CountPoints)
        #assert len(self.tbarmesh.bars) == meshobject.CountEdges, (len(self.tbarmesh.bars), meshobject.CountEdges)
        if btriangleboxing:
