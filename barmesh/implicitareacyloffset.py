...
class DistPZC:
    ...
    def DistPpointPZ(self, p0):
        ...
        if lvflen < self.r:
            self.r = lvflen
            self.v = lv
            assert self.zlo - 1e-8 <= self.p.z + self.v.z <= self.zhi + 1e-8, (self.zlo, self.p.z + self.v.z, self.zhi)
            self.Dpp = [p0]
    ...
    def DistPedgePZF(self, p0z, p1z):
        ...
        if 0 < lam < 1:
            ...
            assert self.zlo -1e-8 <= self.p.z + self.v.z <= self.zhi + 1e-8, (self.zlo, self.p.z, self.p.z + self.v.z, self.zhi)
            self.Dpp = [p0z, p1z]
...
class ImplicitAreaCylOffset:
    ...
    def DistPN(self, pz, n, Bnoedges = False):
        p = n.p
        return self.DistP(pz, p, Bnoedges)
